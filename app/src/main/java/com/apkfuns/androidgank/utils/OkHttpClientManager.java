...
public class OkHttpClientManager {
    ...
    private HttpsDelegate vGetHttpsDelegate() {
        return mHttpsDelegate;
    }

    private DownloadDelegate vGetDownloadDelegate() {
        return mDownloadDelegate;
    }

    private DisplayImageDelegate vGetDisplayImageDelegate() {
        return mDisplayImageDelegate;
    }

    private UploadDelegate vGetUploadDelegate() {
        return mUploadDelegate;
    }


    public static DisplayImageDelegate getDisplayImageDelegate() {
        return getInstance().vGetDisplayImageDelegate();
    }

    public static DownloadDelegate getDownloadDelegate() {
        return getInstance().vGetDownloadDelegate();
    }

    public static UploadDelegate getUploadDelegate() {
        return getInstance().vGetUploadDelegate();
    }

    public static HttpsDelegate getHttpsDelegate() {
        return getInstance().vGetHttpsDelegate();
    }
    ...
    public static void cancelTag(Object tag) {
        getInstance().vCancelTag(tag);
    }

    private void vCancelTag(Object tag) {
    ...
    private static final ResultCallback<String> DEFAULT_RESULT_CALLBACK = new ResultCallback<String>() {
        @Override
    ...
    public class PostDelegate {
        private final MediaType mMediaTypeStream = MediaType.parse("application/octet-stream;charset=utf-8");
        private final MediaType mMediaTypeString = MediaType.parse("text/plain;charset=utf-8");
        ...
        public Response post(String url, String bodyStr, Object tag) throws IOException {
            RequestBody body = RequestBody.create(mMediaTypeString, bodyStr);
        ...
        public Response post(String url, File bodyFile, Object tag) throws IOException {
            RequestBody body = RequestBody.create(mMediaTypeStream, bodyFile);
            Request request = buildPostRequest(url, body, tag);
            Response response = mOkHttpClient.newCall(request).execute();
            return response;
        }
        ...
        public Response post(String url, byte[] bodyBytes, Object tag) throws IOException {
            RequestBody body = RequestBody.create(mMediaTypeStream, bodyBytes);
            Request request = buildPostRequest(url, body, tag);
            Response response = mOkHttpClient.newCall(request).execute();
            return response;
        }
        ...
        public void postAsyn(String url, byte[] bodyBytes, final ResultCallback callback, Object tag) {
            postAsynWithMediaType(url, bodyBytes, MediaType.parse("application/octet-stream;charset=utf-8"),
                    callback, tag);
        }
        ...
        public void postAsyn(String url, File bodyFile, final ResultCallback callback, Object tag) {
            postAsynWithMediaType(url, bodyFile, MediaType.parse("application/octet-stream;charset=utf-8"),
                    callback, tag);
        }
    ...
    public class UploadDelegate {
         ...
         * 异步基于post的文件上传:主方法
         */
        public void postAsyn(String url, String[] fileKeys,
                             File[] files, Param[] params, ResultCallback callback, Object tag) {
            Request request = buildMultipartFormRequest(url, files, fileKeys, params, tag);
            deliveryResult(callback, request);
        }
